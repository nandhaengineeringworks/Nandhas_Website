...
export default function CategoriesDirectoryPage() {
  ...
  const masterCategories = [
    ...
    {
      title: 'Spare Parts & Fittings',
      domain: 'MACHINERY',
      desc: 'Sanitary SS304 valves, tri-clamp fittings, silicone gaskets, and pump impellers.',
      icon: Snowflake,
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-6 sm:space-y-10">
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="flex items-center space-x-2 text-xs text-content-muted overflow-hidden">
            ...
            <span className="font-semibold text-content-main truncate">Master Categories Directory</span>
          </div>
          ...
          <div className="bg-navy-800 rounded-2xl sm:rounded-3xl p-5 sm:p-10 text-white relative overflow-hidden shadow-md">
            <div className="relative z-10 max-w-2xl space-y-2.5 sm:space-y-3">
              <span className="inline-flex items-center text-[10px] sm:text-xs font-bold uppercase tracking-wider text-accent-orange bg-navy-950/80 px-3 py-1 rounded-full border border-navy-700">
                <ShieldCheck className="w-3.5 h-3.5 mr-1.5 text-trust-green shrink-0" /> Complete Catalogue Hierarchy
              </span>
        ...
        {/* Filter Tabs */}
        <div className="flex items-center space-x-2 border-b border-surface-border pb-3 overflow-x-auto custom-scrollbar">
          <button
            onClick={() => setActiveDomain('ALL')}
            className={`px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl text-xs font-bold shrink-0 transition ${
              activeDomain === 'ALL'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {filtered.map((cat, i) => (
            <Link
              key={i}
              href={cat.link}
              className="bg-white rounded-2xl sm:rounded-3xl border border-surface-border overflow-hidden shadow-card hover:shadow-card-hover transition group flex flex-col justify-between"
            >
